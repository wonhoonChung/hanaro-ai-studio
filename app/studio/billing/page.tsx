...
export const metadata = { title: "바나나 · 구독" };
...
const reasonLabel = (r: string) => REASON[r] ?? (r.startsWith("purchase:") ? "바나나 충전" : r.startsWith("refund:") ? "실패 환불" : r.startsWith("job:") ? `사용 (${r.slice(4)})` : r.startsWith("admin:") ? "관리자 조정" : r);
...
export default async function BillingPage({ searchParams }: PageProps<"/studio/billing">) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold">바나나 · 구독</h1>
        <p className="mt-1 text-sm text-muted">1 바나나 = 100원. 충전하거나 월 정액으로 매달 받아서 다섯 제작실에 씁니다.</p>
      </div>

      {sp.ok && <Alert kind="success">구독이 시작되었습니다. 바나나 {p.monthly_credits.toLocaleString()}개가 지급되었습니다.</Alert>}
      {typeof sp.charged === "string" && <Alert kind="success">바나나 {Number(sp.charged).toLocaleString()}개가 충전되었습니다.</Alert>}
      {sp.canceled && <Alert kind="info">구독이 해지되었습니다. 남은 바나나는 계속 쓸 수 있고, 다음 결제는 진행되지 않습니다.</Alert>}
      ...
      <section className="card flex flex-wrap items-center justify-between gap-4 bg-gold-soft border-gold/40">
        <div>
          <p className="text-sm text-[#7a5d00]">내 바나나</p>
          <p className="display text-4xl font-bold text-brand-deep">🍌 {total.toLocaleString()}</p>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <section className="card space-y-4">
          <div>
            <h2 className="font-semibold">바나나 충전</h2>
            <p className="hint">일회성 결제. 많이 살수록 바나나당 단가가 내려갑니다.</p>
          </div>
        ...
        <section className="card space-y-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted mt-1">매월 바나나 {p.monthly_credits.toLocaleString()}개 자동 지급 (바나나당 {Math.round(p.price_krw / p.monthly_credits)}원) · 언제든 해지</p>
            </div>
      ...
      <section className="card">
        <h2 className="font-semibold mb-3">바나나 내역</h2>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {(purchases as BananaPurchase[] | null)?.map((x) => (
                <tr key={x.id} className="border-t border-line"><td className="py-2">{new Date(x.paid_at ?? x.created_at).toLocaleString("ko-KR")}</td><td>바나나 {x.bananas.toLocaleString()}개 충전</td><td className="font-mono text-xs">{x.order_id}</td><td className="text-right">{x.amount.toLocaleString()}원</td></tr>
              ))}
