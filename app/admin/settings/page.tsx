...
export default async function AdminSettings({ searchParams }: PageProps<"/admin/settings">) {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div><h1 className="text-2xl font-bold">요금·ro 설정</h1><p className="mt-1 text-sm text-muted">변경 즉시 요금 안내와 ro 차감에 반영됩니다. 기존 구독자의 다음 결제 금액도 이 값으로 청구됩니다.</p></div>
      ...
      <form action={savePlanSettings} className="card space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div><label className="label">월 ro</label><input name="monthly_credits" type="number" min={0} defaultValue={p.monthly_credits} className="input" required /></div>
        </div>
        <fieldset className="space-y-3">
          <legend className="label">ro 단가</legend>
